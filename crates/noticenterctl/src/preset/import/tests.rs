...
fn import_rejects_outside_css_asset_refs_in_noninteractive_runs() {
    ...
    let error =
        import_preset_into_with_confirm(&import_root.path, &bundle_path, &[], false, |_refs| {
            Err(anyhow!(
                "preset import found CSS asset references outside the UnixNotis config directory"
            ))
        })
