...
impl NotificationList {
    ...
    pub(in crate::ui::notifications) fn trim_to_limits(&mut self) {
    ...
    pub(in crate::ui::notifications) fn insert_entry(
        &mut self,
    ...
    pub(in crate::ui::notifications) fn remove_entry(&mut self, id: u32) {
        if let Some(entry) = self.entries.remove(&id) {
            self.index_remove(&entry.app_key, id, entry.is_active);
        }
