...
pub(super) async fn prepare_trial(
    ...
    let restore_action = stop_active_owner(args, &owner)?;
...
async fn detect_owner(
    ...
    let comm = pid.and_then(read_comm);
    let args = pid.and_then(read_args);
...
fn detect_known_daemons(owner: &Option<OwnerInfo>) -> Vec<DetectedDaemon> {
    ...
    for daemon in KNOWN_DAEMONS {
        let running_pids = pgrep_exact(daemon.name);
        let systemd_active = is_unit_active(daemon.unit);
...
fn stop_active_owner(args: &Args, owner: &OwnerInfo) -> Result<Option<RestoreAction>> {
    let comm = owner
        .comm
        .as_deref()
    ...
    match args.restore {
        RestoreStrategy::None => {
            stop_via_process(pid)?;
            Ok(None)
        }
        RestoreStrategy::Systemd => {
            if !is_unit_active(known.unit) {
                return Err(anyhow!(
                    "systemd restore requested but {} is not active",
                    known.unit
                ));
            }
            stop_via_systemd(known.unit)?;
...
fn stop_via_systemd(unit: &str) -> Result<()> {
...
fn stop_via_process(pid: u32) -> Result<()> {
...
fn read_args(pid: u32) -> Option<Vec<String>> {
