...
#[test]
fn systemd_install_fails_before_enable_when_runtime_unmask_fails() {
    let _lock = lock_env();
    let root = service_flow_root("install-fail-systemd-unmask");
    let log_path = root.join("calls.log");
    let fake_bin = root.join("fake-bin");
    let _fake_tools = write_fake_tools(&fake_bin, &log_path, FakeToolMode::Default);
    let _env = flow_env(&root);
    let _failure = fake_failure_env("systemctl", "unmask");
    let paths = flow_paths(
        &root,
        ServiceManager::systemd_user(
            root.join("home")
    ...
    let error = run_install_and_enable(&paths).expect_err("runtime unmask should fail");

    assert!(error.to_string().contains("clear temporary service mask"));
    let calls = read_calls(&log_path);
    assert!(calls
        .iter()
        .any(|call| call.contains("[--user][--runtime][unmask][unixnotis-daemon.service]")));
    assert!(
        !calls.iter().any(|call| call.contains("[enable][--now]")),
        "service enable must not run after a failed runtime unmask"
    );
    fs::remove_dir_all(root).expect("remove systemd unmask failure fixture");
}
