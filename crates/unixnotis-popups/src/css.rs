//! CSS loading, validation, and hot-reload support for popups.
...
use tracing::warn;
use unixnotis_core::{ThemeConfig, ThemePaths, DEFAULT_BASE_CSS, DEFAULT_POPUP_CSS};
...
impl CssManager {
    pub fn new(theme_paths: ThemePaths, theme_config: ThemeConfig) -> Self {
        Self {
            theme_paths,
            theme_config,
            base: CssProvider::new(),
            popup: CssProvider::new(),
    ...
    pub fn reload(&self, fallback: &str) {
        ...
        load_provider(&self.popup, &self.theme_paths.popup_css, DEFAULT_POPUP_CSS);
        load_override_provider(&self.overrides, &self.theme_config);
    }
...
pub fn start_css_watcher(paths: &ThemePaths, sender: async_channel::Sender<UiEvent>) {
    ...
    for path in [&paths.base_css, &paths.popup_css] {
...
fn load_override_provider(provider: &CssProvider, theme: &ThemeConfig) {
